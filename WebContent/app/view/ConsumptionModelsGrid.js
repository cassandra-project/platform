/*
 * File: app/view/ConsumptionModelsGrid.js
 *
...
Ext.define('C.view.ConsumptionModelsGrid', {
	...
	onGriddragdroppluginBeforeDrop: function(node, data, overModel, dropPosition, dropFunction, options) {
		...
		*/
		...
		if('C.model.'+data.records[0].get('nodeType')==this.store.model.modelName){
			...
			data.copy = true;
			this.store.add({
				'app_id': this.store.navigationNode.parentNode.data.id,
				'name': node.get('name'),
	...
	onButtonClick: function(button, e, options) {
		...
		var app_id = this.store.navigationNode.parentNode.get('id');
		this.store.insert(0, new C.model.ConsumptionModel({app_id: app_id, model:{n:0, params:[]}}));
	...
	onGridpanelBeforeItemDblClick: function(tablepanel, record, item, index, e, options) {
		var myFormCmp = new C.view.ConsumptionModelsForm({});
		Ext.getCmp('MainTabPanel').add(myFormCmp);

		var gridStore = new C.store.ConsumptionModelParams();
		var grid = new C.view.ConsumptionModelParamsGrid({store: gridStore});
		myFormCmp.insert(1, grid);

		var myForm = myFormCmp.getForm();
		myForm.loadRecord(record);
		console.info(record.data.model, "model");
		gridStore.loadData(record.data.model.params[0].values);
		myForm.setValues({
			model_n: record.data.model.n,
			params_n: record.data.model.params[0].n
		});
